...
from baselines.gradher_normalized.util import (
    import_function, store_args, flatten_grads, transitions_in_episode_batch, convert_episode_to_batch_major)
from baselines.gradher_normalized.normalizer import Normalizer
from baselines.gradher_normalized.replay_buffer import ReplayBuffer
...
class DDPG(object):
    ...
    def __init__(self, input_dims, buffer_size, hidden, layers, network_class, polyak, batch_size,
                 ...
                 sample_transitions, gamma, grad_loss_weight, reuse=False, **kwargs):
    ...
    def _create_network(self, reuse=False):
        ...
        grad_target = tf.gradients(tf.reduce_sum(target_tf), target_batch_tf['g'])[0]
        grad_Q = tf.gradients(tf.reduce_sum(self.main.Q_tf), batch_tf['g'])[0]
        self.Q_loss_tf = tf.reduce_mean(tf.square(target_tf - self.main.Q_tf)) -  self.grad_loss_weight*   tf.reduce_mean(batch_tf['r'] * tf.square(grad_target - grad_Q) * self.g_stats.std**2) 
        #self.Q_loss_tf = tf.reduce_mean(tf.square(target_tf - self.main.Q_tf)) +  self.grad_loss_weight*   tf.reduce_mean( tf.square(grad_target - grad_Q)) 
        ...
        self.init_target_net_op = list(
            map(lambda v: v[0].assign(tf.stop_gradient(v[1])), zip(self.target_vars, self.main_vars)))
        self.update_target_net_op = list(
            map(lambda v: v[0].assign(self.polyak * v[0] + (1. - self.polyak) * tf.stop_gradient(v[1])), zip(self.target_vars, self.main_vars)))
