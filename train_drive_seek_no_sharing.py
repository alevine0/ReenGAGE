...
from pooling_td3_policies_without_sharing import PoolingMultiInputPolicy
...
new_logger = configure("./drive_seek_no_sharing/"+name+time_sig+"/training_logs/", ["csv"])
...
model = model_class(
    ...
    dim_per_desired_goal = 2,
    seed=args.seed,
    tensorboard_log= "./drive_seek_no_sharing/"+name+time_sig+"/tensorboard_logs/", **model_extra_args
)

model.set_logger(new_logger)

eval_envs =  [(env_class(**{**env_args, **{"render_skip":100,"render_path":"./drive_seek_no_sharing/"+name+time_sig+"/eval_gifs"}}), "")]
eval_envs[0][0].seed(args.seed+1)
eval_callbacks  = list([EvalCallback(x[0], best_model_save_path="./drive_seek_no_sharing/"+name+x[1]+time_sig+"/",
    log_path="./drive_seek_no_sharing/"+name+x[1]+time_sig+"/eval_logs/", eval_freq=max_episode_length*100, n_eval_episodes=100, render=True) for x in eval_envs])
