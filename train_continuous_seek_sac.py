...
from reengage_sac import GradSAC
...
eval_env_extra_args = {}
eval_env_extra_args["render_path"] = "./continuous_seek_sac/"+name+time_sig+"/eval_gifs"
eval_env_extra_args["render_skip"] = 50



# Initialize the model
new_logger = configure("./continuous_seek_sac/"+name+time_sig+"/training_logs/", ["csv"])
...
model = model_class(
    ...
    seed=args.seed,
    tensorboard_log= "./continuous_seek_sac/"+name+time_sig+"/tensorboard_logs/", 
)
...
eval_callbacks  = list([EvalCallback(x[0], best_model_save_path="./continuous_seek_sac/"+name+x[1]+time_sig+"/",
    log_path="./continuous_seek_sac/"+name+x[1]+time_sig+"/eval_logs/", eval_freq= 2000, n_eval_episodes=50, render=True) for x in eval_envs])
